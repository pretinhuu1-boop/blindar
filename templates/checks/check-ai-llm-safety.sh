...
TMP=$(mktemp)
rg -nE "(openai|anthropic|gemini)\.\w+\.create\(" --type ts "${IGNORE[@]}" -A 10 2>/dev/null | \
  grep -B 10 "\}" | grep -v "max_tokens" > "$TMP" || true
NO_MAX=$(grep -c "create" "$TMP" 2>/dev/null || echo 0)
if [ "$NO_MAX" -gt 0 ]; then
...
TMP=$(mktemp)
rg -nE "['\"][Yy]ou are.*\\\$\{[^}]*input|['\"][Vv]oc[eê].*\\\$\{[^}]*input" --type ts "${IGNORE[@]}" > "$TMP" 2>/dev/null || true
...
TMP=$(mktemp)
rg -nE "(eval|new Function|queryRawUnsafe|innerHTML).*[Cc]ompletion|response\\.content" --type ts "${IGNORE[@]}" > "$TMP" 2>/dev/null || true
...
LLM_ENDPOINTS=$(rg -lE "openai|anthropic" --type ts "${IGNORE[@]}" 2>/dev/null | head -5)
HAS_RATELIMIT=$(rg -lE "(rateLimit|@Throttle|@upstash)" --type ts "${IGNORE[@]}" 2>/dev/null | head -1)
...
TMP=$(mktemp)
rg -nE "(messages|prompt|content).*(\\\$\\{[^}]*(email|cpf|phone|name|password))" --type ts "${IGNORE[@]}" > "$TMP" 2>/dev/null || true
...
TMP=$(mktemp)
rg -nE "tools.*delete|tools.*destroy" --type ts "${IGNORE[@]}" -A 15 2>/dev/null | \
