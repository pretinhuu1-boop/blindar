...
TMP=$(mktemp)
rg -n "(openai|anthropic|gemini)\.\w+\.create\(" --type ts "${IGNORE[@]}" -A 10 2>/dev/null | \
...
TMP=$(mktemp)
rg -n "['\"][Yy]ou are.*\\\$\{[^}]*input|['\"][Vv]oc[eê].*\\\$\{[^}]*input" --type ts "${IGNORE[@]}" > "$TMP" 2>/dev/null || true
...
TMP=$(mktemp)
rg -n "(eval|new Function|queryRawUnsafe|innerHTML).*[Cc]ompletion|response\\.content" --type ts "${IGNORE[@]}" > "$TMP" 2>/dev/null || true
...
LLM_ENDPOINTS=$(rg -l "openai|anthropic" --type ts "${IGNORE[@]}" 2>/dev/null | head -5)
HAS_RATELIMIT=$(rg -l "(rateLimit|@Throttle|@upstash)" --type ts "${IGNORE[@]}" 2>/dev/null | head -1)
...
TMP=$(mktemp)
rg -n "(messages|prompt|content).*(\\\$\\{[^}]*(email|cpf|phone|name|password))" --type ts "${IGNORE[@]}" > "$TMP" 2>/dev/null || true
...
TMP=$(mktemp)
rg -n "tools.*delete|tools.*destroy" --type ts "${IGNORE[@]}" -A 15 2>/dev/null | \
