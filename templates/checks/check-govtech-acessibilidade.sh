...
GOV_HITS=0
GOV_HITS=$(( GOV_HITS + $(rg -c "gov\\.br" --type ts --type js --type html --type json "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
GOV_HITS=$(( GOV_HITS + $(rg -c "vlibras" --type ts --type js --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
GOV_HITS=$(( GOV_HITS + $(rg -c "(lei.de.acesso|\\bLAI\\b|transparencia)" --type ts --type md "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
GOV_HITS=$(( GOV_HITS + $(rg -c "(eMAG|emag)" --type ts --type md --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
GOV_HITS=$(( GOV_HITS + $(rg -c "(prefeitura|ministerio|secretaria.*?estado|tribunal|camara.*?municipal)" --type ts --type md "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
...
ACCESS_BTN=$(rg -c "(acessibilidade|accessibility-?menu|a11y-?menu)" --type ts --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
ACCESSKEY_HITS=$(rg -c "(accesskey|accessKey)" --type ts --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
ALT_SHORTCUTS=$(rg -c "(Alt\\+1|alt\\+1|altKey.*?key.*?[1-4])" --type ts "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
HIGH_CONTRAST=$(rg -c "(alto-?contraste|high-?contrast|highContrast|contrast-mode)" --type ts --type css "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
rg -n "<html" --type ts --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null > "$HTML_TAGS_TMP" || true
...
HTML_ALT_DOC=$(rg -c "\\.(odt|html)\"" --type ts --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
rg -nU "Content-Security-Policy" --type ts --type js --type json --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -A 5 2>/dev/null > "$CSP_HITS_TMP" || true
CSP_BLOCKS_GOV=0
