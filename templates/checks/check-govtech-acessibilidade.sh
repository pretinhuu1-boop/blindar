...
GOV_HITS=0
GOV_HITS=$(( GOV_HITS + $(rg -c "gov\\.br" --type ts --type tsx --type js --type html --type json "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
GOV_HITS=$(( GOV_HITS + $(rg -c "vlibras" --type ts --type tsx --type js --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
GOV_HITS=$(( GOV_HITS + $(rg -c "(lei.de.acesso|\\bLAI\\b|transparencia)" --type ts --type tsx --type md "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
GOV_HITS=$(( GOV_HITS + $(rg -c "(eMAG|emag)" --type ts --type tsx --type md --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
GOV_HITS=$(( GOV_HITS + $(rg -c "(prefeitura|ministerio|secretaria.*?estado|tribunal|camara.*?municipal)" --type ts --type tsx --type md "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0) ))
...
LOGIN_HITS=$(rg -c "(login|signin|sign-in|signIn|auth/login)" --type ts --type tsx "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
GOVBR_LOGIN=$(rg -c "(gov\\.br|govbr|acesso\\.gov|sso\\.acesso)" --type ts --type tsx --type js --type json "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
ACCESS_BTN=$(rg -c "(acessibilidade|accessibility-?menu|a11y-?menu)" --type tsx --type html --type ts "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
ACCESSKEY_HITS=$(rg -c "(accesskey|accessKey)" --type tsx --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
ALT_SHORTCUTS=$(rg -c "(Alt\\+1|alt\\+1|altKey.*?key.*?[1-4])" --type ts --type tsx "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
OUTLINE_NONE=$(rg -nU ":focus[\\s\\S]{0,200}outline\\s*:\\s*(none|0)" --type css --type scss --type tsx --type ts "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
FOCUS_VISIBLE=$(rg -c "(focus-visible|focusVisible|outline:\\s*[12]px)" --type css --type scss --type tsx --type ts "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
HIGH_CONTRAST=$(rg -c "(alto-?contraste|high-?contrast|highContrast|contrast-mode)" --type ts --type tsx --type css --type scss "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
SITEMAP_LINK=$(rg -c "(mapa[- ]?do[- ]?site|site-?map|sitemap)" --type tsx --type html --type ts "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
rg -n "<html" --type tsx --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null > "$HTML_TAGS_TMP" || true
...
HTML_ALT_DOC=$(rg -c "\\.(odt|html)\"" --type tsx --type html --type ts "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
TRANSP_PAGE=$(rg -c "(/transparencia|/dados-abertos|transparencia-ativa)" --type tsx --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
PUBLIC_ORG=$(rg -c "(prefeitura|ministerio|secretaria.*?estado|tribunal|camara.*?municipal|orgao.*?publico)" --type ts --type tsx --type md "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
rg -nU "Content-Security-Policy" --type ts --type tsx --type js --type json --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -A 5 2>/dev/null > "$CSP_HITS_TMP" || true
CSP_BLOCKS_GOV=0
...
VLIBRAS_EMBED=$(rg -c "(vlibras\\.gov\\.br/app|VLibras\\.Widget|vw-access-button)" --type tsx --type html --type js --type ts "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
