...
HAS_MODEL=$(grep -cE "^model\s+(AuditLog|ActivityLog|Audit)" prisma/schema.prisma 2>/dev/null || echo 0)
...
SENSITIVE_OPS=$(rg -lE "(prisma\.user|prisma\.role|prisma\.permission|prisma\.payment).*(update|delete)" --type ts '!node_modules' '!**/*.test.*' 2>/dev/null | wc -l || echo 0)
AUDIT_REFS=$(rg -lE "(auditLog\.create|logAction|writeAudit)" --type ts '!node_modules' '!**/*.test.*' 2>/dev/null | wc -l || echo 0)
