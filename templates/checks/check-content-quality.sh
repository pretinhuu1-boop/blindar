...
IGNORE=(-g '!node_modules' -g '!dist' -g '!build' -g '!.next' -g '!.nuxt' -g '!out' -g '!.svelte-kit' -g '!**/*.test.*')
...
rg -n "(throw new \w*Error|return.*['\"])(undefined|null|NaN|TypeError|ReferenceError|Cannot read|stack)" \
     "${IGNORE[@]}" "${INTEL_GLOBS[@]}" > "$TMP" 2>/dev/null || true
...
VAGUE=$(rg -n "['\"]Tem certeza\?['\"]"   "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
if [ "$VAGUE" -gt 0 ]; then
...
TMP=$(mktemp)
rg -nB 5 -A 5 "(delete|remove|destroy|drop)"   "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | \
...
if has_file ".blindar/copy-style.yml"; then
  ...
  if [ -n "$FORBIDDEN" ]; then
    while read -r word; do
      [ -z "$word" ] && continue
      HITS=$(rg -ci "$word" --type md "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
      if [ "$HITS" -gt 0 ]; then
