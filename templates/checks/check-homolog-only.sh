...
# Os quatro matchers abaixo passam por drop_comment_lines: até a v0.81 um
# comentário no docker-compose falando de `NODE_ENV=development` virava crit,
# e apagar o comentário "resolvia" o achado. Comentário não é config viva.

# 1. NODE_ENV=development no artefato de deploy (CRIT)
DEV_ENV=$(rg -n "NODE_ENV[[:space:]=:'\"]+development" Dockerfile* docker-compose*.y*ml compose*.y*ml Procfile 2>/dev/null | drop_comment_lines | wc -l)
...
DEV_SERVER=$(rg -n "(nodemon|ts-node-dev|next[[:space:]]+dev|npm[[:space:]]+run[[:space:]]+dev|yarn[[:space:]]+dev|pnpm[[:space:]]+dev|vite[[:space:]]*$|flask[[:space:]]+run|uvicorn.*--reload|FLASK_DEBUG|--inspect)" Dockerfile* docker-compose*.y*ml compose*.y*ml Procfile 2>/dev/null | drop_comment_lines | wc -l)
...
DEBUG_ON=$(rg -n "(DEBUG[[:space:]=:'\"]+([Tt]rue|1)|debug[[:space:]]*=[[:space:]]*[Tt]rue)" Dockerfile* docker-compose*.y*ml compose*.y*ml .env.production .env.homolog 2>/dev/null | drop_comment_lines | wc -l)
...
DEV_DB=$(rg -n "(:memory:|sqlite:///?(dev|test)|dev\.db|localhost/.*_dev)" docker-compose*.y*ml compose*.y*ml .env.production .env.homolog 2>/dev/null | drop_comment_lines | wc -l)
if [ "$DEV_DB" -gt 0 ]; then
