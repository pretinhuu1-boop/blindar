...
NO_GUARD=0
TMP=$(mktemp)
rg -n "(app\.(get|post|put|delete|patch)|router\.(get|post|put|delete|patch)|@(Get|Post|Put|Delete|Patch)\()" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null > "$TMP" || true
while IFS=: read -r file line content; do
  [ -z "$file" ] && continue
  ctx=$(sed -n "$((line-3)),$((line+8))p" "$file" 2>/dev/null)
  echo "$ctx" | grep -qE "(auth|@UseGuards|@Roles|requireAuth|isAuthenticated|withAuth|@blindar:public-ok|Depends\(|login_required|permission_classes|IsAuthenticated|current_user|@jwt_required)" || NO_GUARD=$((NO_GUARD+1))
done < "$TMP"
rm -f "$TMP"
...
TMP=$(mktemp)
rg -n "req\.(params|query)\.id" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null > "$TMP" || true
NO_OWN=0
while IFS=: read -r file line content; do
  [ -z "$file" ] && continue
...
HARD_ROLES=$(rg -c "(role|roles)\s*[:=]\s*['\"](admin|root|super)['\"]" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
DEFAULT_ALLOW=$(rg -c "if\s*\(!.*\.(blocked|denied|banned)\)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
