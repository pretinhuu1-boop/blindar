...
# Roles/permissions
ROLES=$(rg -n "(@Roles|UserType|enum Role|role:)" --type ts --type py -g '!node_modules' 2>/dev/null | head -30)
[ -n "$ROLES" ] && EVIDENCE+="=== roles/permissions ===\n$ROLES\n\n"

# Rotas
ROUTES=$(rg -n "(<Route|router\.get|@Get\(|path:)" --type ts --type py -g '!node_modules' 2>/dev/null | head -40)
