...
# Coleta endpoints
ENDPOINTS=""
ENDPOINTS+=$(rg -n "(app|router)\.(get|post|put|delete|patch)\(['\"]" --type ts --type js --type py -g '!node_modules' -g '!dist' 2>/dev/null | head -50)
ENDPOINTS+=$'\n'$(rg -n "@(Get|Post|Put|Delete|Patch)\(" --type ts --type py -g '!node_modules' 2>/dev/null | head -30)
ENDPOINTS+=$'\n'$(find app/api pages/api -name "route.ts" -o -name "*.ts" 2>/dev/null | head -30)

# Coleta chamadas client
CLIENT=""
CLIENT+=$(rg -n "(fetch|axios\.|useQuery|useMutation|trpc\.)" --type ts --type py -g '!node_modules' 2>/dev/null | head -50)
