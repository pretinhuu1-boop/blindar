...
# 1. CORS origin: * (CRIT)
WILD=$(rg -c "(origin:\s*['\"]?\*|Access-Control-Allow-Origin.*\*)" --type ts --type js "${IGNORE[@]}" 2>/dev/null | wc -l || echo 0)
if [ "$WILD" -gt 0 ]; then
...
REFLECT=$(rg -n "credentials:\s*true" --type ts -A 2 "${IGNORE[@]}" 2>/dev/null | grep -E "origin:.*req\.headers|callback\(null,\s*origin" | wc -l || echo 0)
...
HAS_CSRF=$(rg -c "(csurf|csrf-csrf|next-csrf|csrfToken)" --type ts --type js "${IGNORE[@]}" 2>/dev/null | wc -l || echo 0)
HAS_FORMS=$(rg -c "<form\b" --type ts --type html "${IGNORE[@]}" 2>/dev/null | wc -l || echo 0)
...
NO_SAMESITE=$(rg -n "res\.cookie\(|cookies\.set\(" --type ts "${IGNORE[@]}" 2>/dev/null | grep -v "sameSite\|SameSite" | wc -l || echo 0)
