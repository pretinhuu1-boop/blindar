...
# 1. CORS origin: * (CRIT)
WILD=$(rg -c "(origin:\s*['\"]?\*|Access-Control-Allow-Origin.*\*|allow_origins\s*=\s*\[.\*|CORS_ALLOW_ALL_ORIGINS\s*=\s*True)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
if [ "$WILD" -gt 0 ]; then
...
REFLECT=$(rg -n "(credentials:\s*true|allow_credentials\s*=\s*True)" --type ts --type py -A 2 "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -E "origin:.*req\.headers|callback\(null,\s*origin" | wc -l || echo 0)
...
HAS_CSRF=$(rg -c "(csurf|csrf-csrf|next-csrf|csrfToken|CsrfViewMiddleware|CSRFProtect|csrf_token)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
HAS_FORMS=$(rg -c "<form\b" --type ts --type py --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
NO_SAMESITE=$(rg -n "res\.cookie\(|cookies\.set\(" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -v "sameSite\|SameSite" | wc -l || echo 0)
