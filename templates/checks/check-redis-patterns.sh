...
TMP=$(mktemp)
rg -n "redis\.(set|hset|sadd|zadd)\(" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | \
...
TMP=$(mktemp)
rg -ni "(setnx|setNX)\(" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -v "redlock" > "$TMP" || true
...
if [ "$IS_MULTITENANT" -eq 1 ]; then
  ...
  TMP=$(mktemp)
  rg -n "redis\.(get|set|hget|hset|del)\(['\"][^t]" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | \
...
TMP=$(mktemp)
rg -n "new Redis\(\{|new IORedis\(\{" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -A 5 2>/dev/null | grep -v "password\|@blindar:keep" > "$TMP" || true
...
KEYS_STAR=$(rg -c "redis\.keys\(['\"]\*" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
if [ "$KEYS_STAR" -gt 0 ]; then
  add_finding "high" "$KEYS_STAR uso(s) de KEYS * — bloqueia Redis. Usar SCAN" "" ""
fi

# 7. FLUSHALL em código (destrutivo)
FLUSH=$(rg -ci "flushall|flushdb" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
if [ "$FLUSH" -gt 0 ]; then
  add_finding "med" "$FLUSH uso(s) de FLUSHALL/FLUSHDB — apaga tudo, revisar urgente" "" ""
fi

# 8. Pipeline em loop com N round-trips
LOOP_ROUNDTRIPS=$(rg -n "for.*\{[^}]*await redis\." --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
