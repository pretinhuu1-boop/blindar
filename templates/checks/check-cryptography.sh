...
WEAK_HASH=$(rg -c "(crypto\.createHash\(['\"](md5|sha1)['\"]\)|md5\(.*password|sha1\(.*password)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
# 2. bcrypt rounds < 12
LOW_BCRYPT=$(rg -c "bcrypt\.(hash|hashSync)\([^,]+,\s*[0-9]\b" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
[ "$LOW_BCRYPT" -gt 0 ] && add_finding "high" "$LOW_BCRYPT bcrypt com rounds < 10 — usar 12+ (Argon2id melhor)" "" ""

# 3. Cipher fraco (DES, RC4, ECB)
WEAK_CIPHER=$(rg -c "(createCipher\(['\"]des|aes-.*-ecb|rc4)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
DEPRECATED=$(rg -c "crypto\.createCipher\(" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
INSECURE_RANDOM=$(rg -n "Math\.random\(\)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -iE "(token|secret|session|nonce|salt|password|key)" | wc -l)
...
JWT_NONE=$(rg -c "algorithm:\s*['\"]none['\"]" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
[ "$JWT_NONE" -gt 0 ] && { add_finding "crit" "$JWT_NONE JWT com algorithm:none" "" ""; FAIL=1; }

# 7. Hardcoded IV/salt
HARDCODED_IV=$(rg -c "(iv|salt)\s*=\s*['\"][a-zA-Z0-9+/=]{8,}['\"]" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
