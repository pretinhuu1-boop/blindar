...
# Routes
ROUTES=$(rg -nE "(<Route|path:|router\.)" --type ts --type tsx '!node_modules' 2>/dev/null | head -30)
[ -n "$ROUTES" ] && EVIDENCE+="=== routes ===\n$ROUTES\n\n"

# Forms
FORMS=$(rg -nE "(<form|onSubmit|useForm)" --type ts --type tsx '!node_modules' 2>/dev/null | head -20)
[ -n "$FORMS" ] && EVIDENCE+="=== forms (sample) ===\n$FORMS\n\n"

# Botões destrutivos
DESTR=$(rg -nE "(delete|remove|destroy)" --type ts --type tsx '!node_modules' 2>/dev/null | head -20)
