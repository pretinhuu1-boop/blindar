...
# Routes
ROUTES=$(rg -n "(<Route|path:|router\.)" --type ts --type py -g '!node_modules' 2>/dev/null | head -30)
[ -n "$ROUTES" ] && EVIDENCE+="=== routes ===\n$ROUTES\n\n"

# Forms
FORMS=$(rg -n "(<form|onSubmit|useForm)" --type ts --type py -g '!node_modules' 2>/dev/null | head -20)
[ -n "$FORMS" ] && EVIDENCE+="=== forms (sample) ===\n$FORMS\n\n"

# Botões destrutivos
DESTR=$(rg -n "(delete|remove|destroy)" --type ts --type py -g '!node_modules' 2>/dev/null | head -20)
