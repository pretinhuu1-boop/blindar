...
TMP=$(mktemp)
rg -nE "process\.env\.(NEW_|ENABLE_|FEATURE_)[A-Z_]+" --type ts --type js "${IGNORE[@]}" > "$TMP" 2>/dev/null || true
...
HARDCODED=$(rg -cE "if\s*\(\s*(true|false)\s*\)" --type ts "${IGNORE[@]}" 2>/dev/null | wc -l || echo 0)
[ "$HARDCODED" -gt 5 ] && add_finding "med" "$HARDCODED if(true/false) hardcoded — flag morta?" "" ""

# 3. Comment "TEMP" ou "remover depois"
TMP=$(mktemp)
rg -niE "(TEMP|temporary|remover depois|remove later).*flag" --type ts "${IGNORE[@]}" > "$TMP" 2>/dev/null || true
