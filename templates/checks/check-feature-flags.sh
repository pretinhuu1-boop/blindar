...
IGNORE=(-g '!.next' -g '!.nuxt' -g '!out' -g '!.svelte-kit' -g '!node_modules' -g '!dist' -g '!**/*.test.*' -g '!**/*.config.*')
load_intelligence_globs "$BLINDAR_AGENT"

# 1. process.env.NEW_FEATURE / ENABLE inline (anti-pattern)
TMP=$(mktemp)
rg -n "process\.env\.(NEW_|ENABLE_|FEATURE_)[A-Z_]+" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" > "$TMP" 2>/dev/null || true
...
HARDCODED=$(rg -c "if\s*\(\s*(true|false)\s*\)" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
[ "$HARDCODED" -gt 5 ] && add_finding "med" "$HARDCODED if(true/false) hardcoded — flag morta?" "" ""

# 3. Comment "TEMP" ou "remover depois"
TMP=$(mktemp)
rg -ni "(TEMP|temporary|remover depois|remove later).*flag" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" > "$TMP" 2>/dev/null || true
