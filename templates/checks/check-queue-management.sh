...
HAS_QUEUE=$(rg -c "$QUEUE_LIB" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)

# 1. Trabalho pesado/externo inline (email/pdf/relatório/imagem/webhook) sem fila (HIGH)
HEAVY_WORK=$(rg -c "(nodemailer|sendMail|sgMail|@sendgrid|resend\.emails|ses\.send|pdfkit|puppeteer|sharp\(|ffmpeg|generateReport|processImage|sendWebhook)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
if [ "$HAS_QUEUE" -gt 0 ]; then
  HAS_RETRY=$(rg -c "(attempts|backoff|max_retries|maxRetries|retries|retry_policy)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
  ...
  HAS_DLQ=$(rg -c "(deadLetter|dead-letter|DLQ|failed.*queue|onFailed|removeOnFail)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
  ...
  # 4. Idempotência (MED)
  HAS_IDEMPOTENT=$(rg -c "(idempoten|jobId|dedup|deduplication|uniqueKey)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
