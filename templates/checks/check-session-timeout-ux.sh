...
HAS_AUTH=$(rg -c "(session|login|signin|jwt|auth|Cookie|useAuth|AuthProvider)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
HAS_IDLE=$(rg -c "(idle|inactivity|sessionTimeout|session-timeout|autoLogout|auto-logout|IdleTimer|useIdle|onIdle|InactivityTimer)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
HAS_CONFIG=$(rg -c "(SESSION_TIMEOUT|INACTIVITY_TIMEOUT|idleTimeoutMinutes|timeoutMinutes|sessionTimeoutMs|IDLE_TIMEOUT)" --type ts --type js -g '.env*' "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
HAS_BLUR=$(rg -c "(blur|backdrop|overlay|modal.*expir|expir.*modal|SessionExpired|TimeoutModal)" --type ts --type js --type css "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
HAS_RESUME=$(rg -c "(autosave|auto-save|draft|persistState|localStorage.*form|beforeunload|restoreState|resume)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
