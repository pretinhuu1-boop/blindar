...
TMP=$(mktemp)
rg -n "@Cron\(" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -A 8 2>/dev/null | \
...
TMP=$(mktemp)
rg -n "queue\.add\(" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -A 5 2>/dev/null | \
...
HAS_WATCHDOG=$(rg -l "(cron_runs|cronRun|watchdog|silent_failure)" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | head -1)
...
if grep -qE "bullmq" package.json 2>/dev/null; then
  HAS_DLQ=$(rg -l "(getFailed|DLQ|dead.letter)" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | head -1)
  if [ -z "$HAS_DLQ" ]; then
...
TMP=$(mktemp)
rg -n "setInterval\(" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -A 3 2>/dev/null | grep -v "clearInterval" > "$TMP" || true
