...
if [ "$HELMET" -eq 0 ] && [ "$NEXT_HEADERS" -eq 0 ]; then
  ...
  for h in "Content-Security-Policy" "Strict-Transport-Security" "X-Frame-Options" "X-Content-Type-Options" "Referrer-Policy" "Permissions-Policy"; do
    PRESENT=$(rg -c "$h" --type ts --type js --type py --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
...
UNSAFE_INLINE=$(rg -c "unsafe-inline" --type ts --type js --type py --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
[ "$UNSAFE_INLINE" -gt 0 ] && add_finding "high" "$UNSAFE_INLINE CSP com 'unsafe-inline' — usar nonce/hash" "" ""

# CSP com 'unsafe-eval'
UNSAFE_EVAL=$(rg -c "unsafe-eval" --type ts --type js --type py --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
