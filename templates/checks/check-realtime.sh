...
TMP=$(mktemp)
rg -n "io\.on\(['\"]connection" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -A 8 2>/dev/null | grep -v "auth\|verify\|jwt" > "$TMP" || true
...
TMP=$(mktemp)
rg -n "io\.emit\(" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -v "tenant:\|user:\|room:" > "$TMP" || true
...
URL_TOKEN=$(rg -c "io\(['\"]ws://.*token=" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
[ "$URL_TOKEN" -gt 0 ] && add_finding "high" "Token em URL WS (vaza em logs/proxies)" "" ""

# 4. Sem heartbeat config
HAS_HEARTBEAT=$(rg -l "(pingInterval|pingTimeout|heartbeat)" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | head -1)
