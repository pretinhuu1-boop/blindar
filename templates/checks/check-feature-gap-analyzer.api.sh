...
# Endpoints
ENDP=$(rg -n "(@(Get|Post|Put|Delete|Patch)|router\.(get|post)|app\.(get|post))" --type ts -g '!node_modules' 2>/dev/null | head -40)
...
# Feature flags
FLAGS=$(rg -n "(feature_flag|flagsmith|growthbook|process\.env\.FEATURE)" --type ts -g '!node_modules' 2>/dev/null | head -10)
