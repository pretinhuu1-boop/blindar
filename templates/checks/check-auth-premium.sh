...
TMP=$(mktemp)
rg -n "(algorithm|alg)[:\s]*['\"]HS256['\"]" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" > "$TMP" 2>/dev/null || true
...
if rg -l "refreshToken|refresh_token" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | head -1 | grep -q .; then
  if ! rg -l "(rotation|rotate|used_at|family_id)" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | head -1 | grep -q .; then
...
TMP=$(mktemp)
rg -n "argon2\.(hash|verify)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -A 10 2>/dev/null > "$TMP" || true
