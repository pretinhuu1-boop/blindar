...
EXTERNAL_CALL=$(rg -c "(fetch\(|axios|http\.request|https\.request|got\(|node-fetch|undici|requests\.(get|post|put)|httpx|urllib)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)

if [ "$EXTERNAL_CALL" -gt 0 ]; then
  # 1. Chamada externa sem timeout (HIGH) — trava o sistema quando o upstream pendura
  HAS_TIMEOUT=$(rg -c "(timeout|AbortController|AbortSignal|signal:|AbortSignal\.timeout|Timeout=|connect_timeout|read_timeout)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
  ...
  HAS_CIRCUIT=$(rg -c "(opossum|cockatiel|brakes|circuit.?breaker|resilience4j|hystrix|pybreaker|CircuitBreaker)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
  ...
  HAS_RETRY=$(rg -c "(p-retry|async-retry|retry\(|backoff|tenacity|@Retryable|exponential)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
