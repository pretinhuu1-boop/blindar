...
EVAL_USER=$(rg -n "((^|[^.\w])(eval|exec)\(|new Function\().*\b(req\.|request\.|input|userInput|params)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
DANGEROUS=$(rg -n "dangerouslySetInnerHTML=\{\{\s*__html:\s*[a-z][a-zA-Z]*\b" --type ts --type py 2>/dev/null | grep -vE "(sanitiz|DOMPurify)" | wc -l)
[ "$DANGEROUS" -gt 0 ] && add_finding "high" "$DANGEROUS dangerouslySetInnerHTML sem sanitização" "" ""

# 3. innerHTML = userInput
INNER_HTML=$(rg -n "\.innerHTML\s*=\s*[^'\"]" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -vE "(sanitiz|DOMPurify|@blindar:keep)" | wc -l)
...
SQL_CONCAT=$(rg -n "(SELECT|INSERT|UPDATE|DELETE).*(\+|%s|\.format\(|f[\"']).*\b(req\.|request\.|userInput|input|uid|params)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
[ "$SQL_CONCAT" -gt 0 ] && add_finding "crit" "$SQL_CONCAT SQL com concat de user input — SQL injection" "" ""

# 5. execSync/exec com user input
SHELL_INJ=$(rg -n "((^|[^.\w])(exec|execSync|spawn)\(|os\.system\(|subprocess\.(run|call|Popen)\().*(\b(req\.|request\.|userInput|input)|shell\s*=\s*True)" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -vE "@blindar:safe-exec" | wc -l)
...
if grep -qE "\"express\":" package.json 2>/dev/null; then
  HAS_HELMET=$(rg -c "helmet" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
  [ "$HAS_HELMET" -eq 0 ] && add_finding "high" "Express sem helmet middleware" "" ""
fi

# 7. Open redirect (res.redirect com user input)
OPEN_REDIR=$(rg -n "res\.redirect\(.*\breq\." --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -v "whitelist\|allowlist" | wc -l)
[ "$OPEN_REDIR" -gt 0 ] && add_finding "med" "$OPEN_REDIR res.redirect com user input — open redirect" "" ""

# 8. document.write (XSS surface)
DOC_WRITE=$(rg -c "document\.write\(" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
