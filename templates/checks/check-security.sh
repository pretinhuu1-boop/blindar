...
EVAL_USER=$(rg -nE "(eval|new Function)\(.*\b(req\.|input|userInput)" --type ts --type js "${IGNORE[@]}" 2>/dev/null | wc -l)
...
DANGEROUS=$(rg -nE "dangerouslySetInnerHTML=\{\{\s*__html:\s*[a-z][a-zA-Z]*\b" --type ts 2>/dev/null | grep -vE "(sanitiz|DOMPurify)" | wc -l)
[ "$DANGEROUS" -gt 0 ] && add_finding "high" "$DANGEROUS dangerouslySetInnerHTML sem sanitização" "" ""

# 3. innerHTML = userInput
INNER_HTML=$(rg -nE "\.innerHTML\s*=\s*[^'\"]" --type ts --type js "${IGNORE[@]}" 2>/dev/null | grep -vE "(sanitiz|DOMPurify|@blindar:keep)" | wc -l)
...
SQL_CONCAT=$(rg -nE "(SELECT|INSERT|UPDATE|DELETE).*\+.*\b(req\.|userInput|input)" --type ts --type js "${IGNORE[@]}" 2>/dev/null | wc -l)
[ "$SQL_CONCAT" -gt 0 ] && add_finding "crit" "$SQL_CONCAT SQL com concat de user input — SQL injection" "" ""

# 5. execSync/exec com user input
SHELL_INJ=$(rg -nE "(exec|execSync|spawn)\(.*\b(req\.|userInput|input)" --type ts --type js "${IGNORE[@]}" 2>/dev/null | grep -vE "@blindar:safe-exec" | wc -l)
...
OPEN_REDIR=$(rg -nE "res\.redirect\(.*\breq\." --type ts --type js "${IGNORE[@]}" 2>/dev/null | grep -v "whitelist\|allowlist" | wc -l)
