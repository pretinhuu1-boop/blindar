...
EVAL_USER=$(rg -n "(eval|new Function)\(.*\b(req\.|input|userInput)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
...
INNER_HTML=$(rg -n "\.innerHTML\s*=\s*[^'\"]" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -vE "(sanitiz|DOMPurify|@blindar:keep)" | wc -l)
...
SQL_CONCAT=$(rg -n "(SELECT|INSERT|UPDATE|DELETE).*\+.*\b(req\.|userInput|input)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
[ "$SQL_CONCAT" -gt 0 ] && add_finding "crit" "$SQL_CONCAT SQL com concat de user input — SQL injection" "" ""

# 5. execSync/exec com user input
SHELL_INJ=$(rg -n "(exec|execSync|spawn)\(.*\b(req\.|userInput|input)" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -vE "@blindar:safe-exec" | wc -l)
...
if grep -qE "\"express\":" package.json 2>/dev/null; then
  HAS_HELMET=$(rg -c "helmet" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
  [ "$HAS_HELMET" -eq 0 ] && add_finding "high" "Express sem helmet middleware" "" ""
fi

# 7. Open redirect (res.redirect com user input)
OPEN_REDIR=$(rg -n "res\.redirect\(.*\breq\." --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -v "whitelist\|allowlist" | wc -l)
[ "$OPEN_REDIR" -gt 0 ] && add_finding "med" "$OPEN_REDIR res.redirect com user input — open redirect" "" ""

# 8. document.write (XSS surface)
DOC_WRITE=$(rg -c "document\.write\(" --type ts --type js "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l)
