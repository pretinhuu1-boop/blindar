...
NAIVE_NOW=$(rg -c "datetime\.utcnow\(\)" --type py "${IGNORE[@]}" 2>/dev/null | wc -l)
...
DT_NAIVE=$(rg -c "Column\([^)]*DateTime\b" --type py "${IGNORE[@]}" 2>/dev/null | wc -l)
DT_AWARE=$(rg -c "DateTime\(timezone=True\)|DateTime\(timezone = True\)" --type py "${IGNORE[@]}" 2>/dev/null | wc -l)
