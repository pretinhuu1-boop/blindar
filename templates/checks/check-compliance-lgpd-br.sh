...
LGPD_ENDPOINTS=$(rg -c "/api/lgpd/|/lgpd/(export|delete|access|correct|anonymize|opposition)" --type ts 2>/dev/null | wc -l || echo 0)
...
HAS_AGE_GATE=$(rg -n "(birthDate|date_of_birth|idade).*[<>=]?\s*1[3-8]" --type ts 2>/dev/null | wc -l || echo 0)
