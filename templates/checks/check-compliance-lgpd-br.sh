...
HAS_BR_PII=$(rg -l "(cpf|cnpj|cep|rg)" --type ts --type py --type prisma 2>/dev/null | head -1)
...
LGPD_ENDPOINTS=$(rg -c "/api/lgpd/|/lgpd/(export|delete|access|correct|anonymize|opposition)" --type ts --type py 2>/dev/null | wc -l || echo 0)
...
HAS_BANNER=$(rg -l "(cookie.*consent|cookieConsent|CookieBanner|Klaro)"   2>/dev/null | head -1)
if [ -z "$HAS_BANNER" ] && rg -l "(analytics|gtag|fbq)"  2>/dev/null | head -1 | grep -q .; then
  add_finding "high" "Analytics sem cookie banner — LGPD Art. 7-9 violação" "" ""
fi

# 6. Gate Art. 14 (menores) em cadastro
HAS_AGE_GATE=$(rg -n "(birthDate|date_of_birth|idade).*[<>=]?\s*1[3-8]" --type ts --type py 2>/dev/null | wc -l || echo 0)
...
HAS_ANON=$(rg -l "(anonymize|anonimiz|md5.*email|hash.*email).*lgpd" --type ts --type py 2>/dev/null | head -1)
