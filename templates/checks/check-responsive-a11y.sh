...
TMP=$(mktemp)
rg -nP "<img(?![^>]*\balt=)" --type html "${IGNORE[@]}" "${INTEL_GLOBS[@]}" > "$TMP" 2>/dev/null || true
...
TMP=$(mktemp)
rg -nU "<button[^>]*>\s*<svg"   "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -v "aria-label" > "$TMP" || true
...
TMP=$(mktemp)
rg -nU "<input[^>]*placeholder="   "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | grep -v "aria-label\|<label" > "$TMP" || true
...
TMP=$(mktemp)
rg -n "text-overflow\s*:\s*ellipsis" --type css "${IGNORE[@]}" "${INTEL_GLOBS[@]}" > "$TMP" 2>/dev/null || true
