...
TMP=$(mktemp)
rg -nU "['\"]Olá[^'\"]{30,}['\"]" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" -g '!**/templates/**' -g '!**/locales/**' > "$TMP" 2>/dev/null || true
...
TMP=$(mktemp)
rg -n "(resend|ses|sendgrid|nodemailer)\.\w+\.send\(" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" > "$TMP" 2>/dev/null || true
SENDS=$(wc -l < "$TMP" || echo 0)
SUPPRESSED_CHECK=$(rg -l "(isSuppressed|email_suppressions|supress)" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | head -1)
...
HAS_BOUNCE=$(rg -l "(bounce|complaint).*webhook|@Post.*bounce" --type ts --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | head -1)
...
NOREPLY=$(rg -ci "no-reply@|noreply@" --type ts --type js --type py "${IGNORE[@]}" "${INTEL_GLOBS[@]}" 2>/dev/null | wc -l || echo 0)
