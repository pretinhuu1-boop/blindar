...
EMPTY_FN=$(rg -c "(functions|tasks|jobs|processors|handlers)\s*[:=]\s*\[\s*\]" --type py --type ts --type js "${IGNORE[@]}" 2>/dev/null | wc -l)
...
BULL_NOPROC=$(rg -n "new Worker\(\s*['\"][^'\"]+['\"]\s*\)" --type ts --type js "${IGNORE[@]}" 2>/dev/null | wc -l)
