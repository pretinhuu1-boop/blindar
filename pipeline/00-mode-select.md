...
title: Seleção de modo de operação — GREENFIELD / HARDEN / EVOLVE / RECOVERY
...
> O projeto precisa ser **criado**, **blindado**, **evoluído** ou **consertado**?
...
Nenhuma das anteriores. Projeto existente, saudável, ainda não em produção —
ou em produção mas o operador quer o pipeline completo mesmo assim.
...
| | GREENFIELD | HARDEN | EVOLVE | RECOVERY |
|---|---|---|---|---|
| Pipeline | `pipeline/GREENFIELD.md` | 00→09 (padrão) | 00→09 com travas | `pipeline/RECOVERY.md` |
| Gate de suite vermelha (Fase 1) | N/A (não há suite ainda) | **PARA** | **PARA** | **é a condição de entrada** |
| Gate de repo sujo (Fase 1) | ignora | PARA | PARA | ignora |
| Modo default | supervised | auto | **supervised** | supervised |
| Round máximo | livre (construção) | ≤80 LOC | **≤40 LOC** | o mínimo p/ estabilizar |
| Troca de tecnologia | livre (nada existe) | permitida c/ ADR | **proibida sem ADR + aprovação** | proibida |
| Migration destrutiva | livre | pede autorização | **proibida** | proibida |
| Módulo 19 (pentest ativo) | N/A | c/ autorização | **nunca contra produção** | nunca |
...
```yaml
operation_mode: harden   # greenfield | harden | evolve | recovery
